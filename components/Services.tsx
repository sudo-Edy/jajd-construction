        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service) => (
              <div key={service.id} className="group relative cursor-pointer" onClick={onOpenQuote}>
                 {/* Ambilight / Bloom Effect */}
                 <div 
                    className="absolute inset-0 translate-y-4 bg-cover bg-center rounded-2xl opacity-0 group-hover:opacity-40 blur-xl transition-all duration-500 pointer-events-none"
                    style={{ 
